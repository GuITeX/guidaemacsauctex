...
In questa guida tematica parleremo del programma GNU Emacs (ovvero del programma
\emacs\ del progetto GNU, mentre non si accennerà alle sue diverse varianti) e
del pacchetto \auctex.  Si spiegherà come installarli e configurarli sui tre
principali sistemi operativi in circolazione (Windows, GNU/Linux e Mac~OS), e
come usarli per scrivere documenti \LaTeX.  Per finire si darà qualche
indicazione per personalizzare e rendere più efficiente \emacs.
...
\keys{\ctrl} verrà indicato con \verb!C!, l'\,\keys{Invio} con \verb!RET!,
mentre \verb!M! indica il tasto \keys{\Meta} (sulle moderne tastiere
quest'ultimo tasto è scomparso, ma si può ottenere lo stesso risultato con
\keys{\Alt} oppure premendo e rilasciando \keys{\esc}\,).  Nella notazione di
queste scorciatoie, il trattino che separa due tasti in una combinazione di
tasti indica che i tasti vanno premuti contemporaneamente.  Così, quando si dirà
che per eseguire un comando bisogna usare la combinazione \verb!M-x!, si
dovranno premere \emph{contemporaneamente} \keys{\Alt} e \keys{x} oppure premere
\keys{\esc}\,, rilasciarlo e premere \emph{successivamente} \keys{x}\,.
...
\begin{figure}[ht]
  ...
  \end{tikzpicture}
  \caption{Un \emph{frame} di GNU Emacs che contiene la classica schermata
    iniziale.}
...
Emacs chiederà di aprire/trovare un file.  Scrivendo un nome che nella cartella
di lavoro non esiste questo file sarà creato. Dopo averlo nominato, per esempio
\filestyle{primo.tex}, si può cominciare a scrivere in \LaTeX. Per gli amanti
delle scorciatoie da tastiera (autentico punto di forza di \emacs), scrivendo
\verb|C-c C-e| \emacs\ chiederà quale ambiente inserire.  Il cursore si sposterà
nel \emph{minibuffer} e lì si potrà scrivere il nome dell'ambiente da inserire.
...
Le macro \LaTeX\ possono, come è naturale che sia, essere inserite manualmente,
...
documenti \LaTeX{}.  Per modificare le impostazioni relative al pacchetto
\auctex{} è possibile eseguire la funzione
\verb!M-x customize-group RET AUCTeX RET!.  In questo modo si aprirà un buffer
in cui modificare le opzioni desiderate tramite l'interfaccia.
