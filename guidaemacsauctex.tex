...
\ProvidesFile{guidaemacsauctex.tex}[2013/03/09 v.1.0 Breve guida all'uso di
Emacs per LaTeX]
...
\title{Guida pratica all'uso\\ di\\ GNU Emacs \& AUC\TeX}
...
utilizzare i gestori di pacchetti a interfaccia grafica, riportiamo qui i
comandi da terminale che possono essere eseguiti per installare \emacs{} in
alcune delle principali distribuzioni.  Per Debian e Ubuntu:
\begin{lstlisting}
...
pacchetti della propria distribuzione.  Ecco i comandi da usare per installare
da terminale il pacchetto nelle principali distribuzioni.  Debian e Ubuntu:
\begin{lstlisting}
...
Su Debian e Ubuntu, il gestore pacchetti potrebbe raccomandare l'installazione
della distribuzione TeX Live presente nei depositi ufficiali del sistema
operativo.  Per evitare che questo avvenga, per esempio perché si utilizza una
distribuzione TeX Live installata in altro modo, se si installa il pacchetto da
terminale è sufficiente aggiungere l'opzione \verb!--no-install-recommends!
\begin{lstlisting}
...
distribuzione.  Il dizionario italiano di GNU Aspell si chiama
\progstyle{aspell-it} quindi da terminale può essere installato in Debian con il
comando
\begin{lstlisting}
...
la finestra principale come in figura~\ref{fig:principale}.  Nel gergo di
\emacs\ una finestra è chiamata \emph{frame}.  Ciascun \emph{frame} può essere
suddiviso in una o più \emph{windows}, come quella rappresentata nella stessa
figura, cioè la zona di principale interazione con \emacs, vale a dire lo spazio
dove l'utente scrive.  Un'altra componente fondamentale di un \emph{frame} di
Emacs, mostrata nella figura, è il \emph{minibuffer}.  Il \emph{minibuffer} è il
posto dove \emacs\ dialoga con l'utente mostrando messaggi di notifica oppure
...
Ci sono due modi per creare un nuovo documento: si può andare nel menu
...
da inserire, questo è messaggio visualizzato:
\begin{lstlisting}
...
gli argomenti e le opzioni che queste macro accettano, infatti usando
\verb!C-c RET frac RET! nel documento si otterrà \verb!\frac{}{}! e il cursore
si posizionerà all'interno del primo paio di parentesi graffe.  Allo stesso
modo, usando \verb!C-c RET sqrt RET! verrà richiesto l'ordine \verb!n! della
radice da inserire (premere direttamente \verb!RET! per non inserire nulla) e si
otterrà \verb!\sqrt[n]{}!.  Un altro esempio è la macro \verb|\verb|: \auctex{}
chiederà di scegliere il carattere da utilizzare come delimitatore e poi di
inserire il testo di argomento.
...
Prima di andare avanti è bene specificare che le variabili di cui parleremo sono
però messe a disposizione da \auctex. La riga \verb|%%% mode: latex| indica che
...
si deve ricaricare il file.  Questo può essere fatto aprendo e chiudendo \emacs,
oppure, più semplicemente, eseguendo \verb|M-x revert-buffer RET yes RET|.
...
citate variabili locali \verb|mode: latex| e \verb|TeX-master: t| si aggiungono
diverse altre che risultano essere molto utili in un documento \filestyle{.tex}.
Alcune di queste sono messe a disposizione da \emacs\ e altre da \auctex.
...
relative al pacchetto \auctex{} è possibile eseguire la funzione
\verb!M-x customize-group RET AUCTeX RET!.  In questo modo si aprirà un buffer
nel quale sarà possibile modificare, tramite l'interfaccia, le opzioni
desiderate.
...
Per fare in modo che \auctex\ conosca quali pacchetti \LaTeX\ sono stati
caricati, e di conseguenza fornire le funzioni di autocompletamento per le
macro, bisogna permettergli di effettuare il parsing dei propri documenti e
questo è possibile aggiungendo nel proprio file \filestyle{.emacs} il seguente
codice
\begin{lstlisting}
