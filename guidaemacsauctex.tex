...
\ProvidesFile{guidaemacsauctex.tex}[2013/05/29 v.1.1.3 Breve guida all'uso di Emacs per LaTeX]
...
codice, cioè si deve aggiungere al proprio file di inizializzazione il codice
\begin{lstlisting}
(setq TeX-source-correlate-mode t
      TeX-source-correlate-start-server t
      TeX-view-program-selection
	...
	(output-html "xdg-open")))
\end{lstlisting}
% NOTA: non riesco a far funzionare la ricerca diretta.
In \progstyle{Okular}, nel menu \menu{Impostazioni > Configura Okular...},
scheda ``Editor'', selezionare ``Emacs client'' nel menu a discesa.  A questo
punto sarà possibile eseguire la ricerca inversa da \progstyle{Okular} a
\emacs{} tenendo premuto \keys{\shift} mentre si fa clic con il tasto sinistro
del mouse.
