...
\auctex\ è invece un pacchetto per \emacs\ che estende notevolmente le
funzionalità di \emacs\ per scrivere documenti in \LaTeX\ ed in altri formati
legati al programma di tipocomposizione \TeX.
...
repository ufficiale presente sul sito \url{https://github.com/} alla pagina
\url{https://github.com/GuITeX/guidaemacsauctex}. Per compilare il codice
sorgente si consigli di munirsi di una distribuzione \LaTeX\ aggiornata. È
inoltre necesario scaricare ed installare nel proprio albero personale la classe
...
documento \filestyle{.tex} semplice e poi uno più complesso composto da più file.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}\footnotesize\sffamily
    ...
    % minibuffer
    % 
    \draw[line] (8.2,0.15) -- (9,0.15);
    \node[block] at (10,0.15) {minibuffer};
    % 
    % buffer
    % 
    \draw[line] (8.2,3.5) -- (9,3.5);
    \node[block] at (10,3.5) {buffer};
    % 
    % toolbar
    % 
    \draw[line] (8.2,7) -- (10,7) -- (10,6.5);
    \node[block] at (10,6.2) {toolbar};
    % 
    % menubar
    % 
    \draw[line] (8.2,7.4) -- (9,7.4);
    \node[block] at (10,7.4) {menubar};
  \end{tikzpicture}
