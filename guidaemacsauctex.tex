...
\begin{longtable}{>{\ttfamily}l>{\ttfamily}lp{0.35\textwidth}}
  ...
  \caption{Scorciatoie di alcuni comuni comandi di \emacs.  \emph{Scorciatoia} è
    la combinazione di tasti da premere per ottenere il risultato descritto
    nell'ultima colonna.  La seconda colonna riporta la \emph{funzione}, da
    eseguire con \texttt{M-x}, a cui è legata quella particolare combinazione di
    tasti.}
...
Un'attenzione particolare meritano le scorciatoie specifiche di \auctex{}, che
sono quindi utilizzabili solo quando si lavora in un documento \TeX{} o
\LaTeX{}.  La tabella~\ref{tab:auctex-cheat-sheet} riporta le principali
scorciatoie per eseguire un comando sul file (come compilazione, visualizzazione
del documento finale), inserire comandi, selezionare il carattere e formattare
il codice sorgente.

Anche in questo caso, non è necessario impararle tutti subito a memoria, la
pratica aiuterà a memorizzarle, però facciamo notare che la maggior parte di
queste scorciatoie sono effettivamente molto facili da ricordare grazie alla
loro denominazione logica.  Le più comuni scorciatoie sono del tipo \texttt{C-c
  C-\meta{key}}, in cui \meta{key} è una lettera che richiama la funzione da
eseguire.  Così, per esempio, \texttt{C-c C-s} serve per inserire un nuovo
comando di \emph{\textbf{s}ezionamento}.

\begin{longtable}{>{\small\ttfamily}l>{\small\ttfamily}lp{0.3\textwidth}}
  % intestazione iniziale
  \caption{Scorciatoie dei principali comandi specifici di \auctex{}.
    \emph{Scorciatoia} è la combinazione di tasti da premere per ottenere il
    ...
    particolare combinazione di tasti.  Nella terza colonna è riportato
    l'effetto della scorciatoia, insieme alla parola chiave inglese utile per
    ricordarla facilmente.}
  \label{tab:auctex-cheat-sheet} \\
  \toprule
  {\normalfont\scshape Scorciatoia} & {\normalfont\scshape Funzione} &
  \textsc{Effetto} \\
  \midrule
  ...
  \endlastfoot
  % corpo della tabella
  \multicolumn{3}{c}{Compilazione e visualizzazione} \\
  \midrule
  C-c C-c & TeX-command-master & Esegue diversi comandi sul documento attuale:
  compila, esegue il motore bibliografico, compila l'indice analitico, elimina i
  file temporanei, eccetera.  Parola chiave: \emph{\textbf{c}ommand}
  (comando) \\
  C-c C-v & TeX-view & Apre il documento finale con il visualizzatore esterno.
  Parola chiave: \emph{\textbf{v}iew} (visualizza) \\
  C-c C-a & TeX-command-run-all & Compila tutto il documento fino a quando è
  pronto e apre il visualizzatore se non ci sono errori.  Parola
  chiave: \emph{\textbf{a}ll} (tutto) \\
  \midrule
  \multicolumn{3}{c}{Inserimento di comandi} \\
  \midrule
  C-c C-e & LaTeX-environment & Inserisce un nuovo ambiente \LaTeX{}, con
  autocompletamento del nome e degli eventuali argomenti opzionali.  Parola
  chiave: \emph{\textbf{e}nvironment} (ambiente) \\
  C-u C-c C-e & & Sostituisce l'ambiente \LaTeX{} attuale con uno nuovo \\
  C-c C-s & LaTeX-section & Inserisce un nuovo comando di sezionamento \LaTeX{}
  (\verb|\chapter|, \verb|\section|, eccetera) con autocompletamento del nome.
  Parola chiave: \emph{\textbf{s}ection} (sezione) \\
  C-c C-m & TeX-insert-macro & Inserisce una macro con autocompletamento del
  nome e degli argomenti, quando possibile.  Si può eseguire anche con
  \texttt{C-c RET}.  Parola chiave: \emph{\textbf{m}acro} (comando \TeX{}) \\
  \midrule
  \multicolumn{3}{c}{Selezione del carattere} \\
  \midrule
  C-c C-f C-b & & Inserisce testo in \textbf{neretto}.  Parole chiave:
  \emph{\textbf{b}old \textbf{f}ont} \\
  C-c C-f C-i & & Inserisce testo in \textit{corsivo}.  Parole chiave:
  \emph{\textbf{i}talics \textbf{f}ont} \\
  C-c C-f C-r & & Inserisce testo con \textrm{carattere romano} (o scrittura
  rotonda, diritto).  Parole chiave: \emph{\textbf{r}oman \textbf{f}ont} \\
  C-c C-f C-e & & Inserisce testo \emph{enfatizzato}.  Parole chiave:
  \emph{\textbf{e}mphasized \textbf{f}ont} \\
  C-c C-f C-t & & Inserisce testo con carattere a \texttt{spaziatura fissa}.
  Parole chiave: \emph{\textbf{t}ypewriter \textbf{f}ont} \\
  C-c C-f C-s & & Inserisce testo con \textsl{carattere obliquo}.  Parole
  chiave: \emph{\textbf{s}lanted \textbf{f}ont} \\
  C-c C-f C-c & & Inserisce testo in \textrm{maiuscoletto}.  Parole chiave:
  \emph{small \textbf{c}aps \textbf{f}ont} \\
  C-c C-f C-d & & Rimuove il comando di selezione del carattere attorno al
  cursore.  Parole chiave: \emph{\textbf{d}elete \textbf{f}ont} \\
  C-u C-c C-f C-\meta{key} & & Sostituisce il comando di selezione del carattere
  attorno al cursore con un font specificato da \meta{key} (\texttt{C-b} per il
  neretto, \texttt{C-i} per l'italico, \texttt{C-r} per il romano, eccetera) \\
  \midrule
  \multicolumn{3}{c}{Formattazione del codice sorgente} \\
  \midrule
  TAB & indent-for-tab-command & Indenta il codice \\
  M-q & fill-paragraph & Riformatta il capoverso corrente \\
  C-c . & LaTeX-mark-environment & Seleziona l'ambiente \LaTeX{} corrente \\
  C-c * & LaTeX-mark-section & Seleziona la sezione \LaTeX{} corrente \\
  C-c ; & & Commenta o decommenta la regione
  selezionata \\
  C-c \% & & Commenta o decommenta il
  capoverso corrente
\end{longtable}
