...
Su ogni sistema sistema operativo si ha la possibilità di aprire \emacs\ a partire
da un collegamento al programma principale. Aperto \emacs, si dovrebbe avere davanti
...
Per creare un nuovo documento basta premere \verb|C-c C-f| ed Emacs chiederà di
aprire/trovare un file. Scrivendo un nome che nella cartella di lavoro non
esiste questo file sarà creato. Scritto il nome del file, per esempio
...
questo punto non c'è alcun documento attivo, l'ambiente \amb{document} sarà
...
\item scrivendo \verb|C-c C-c|. Solo in questo caso nel minibuffer si avrà
\begin{lstlisting}
...
\end{lstlisting}
Con questa procedura si scrive un nuovo documento in \LaTeX. Sembra difficile, e
molti reputano \emacs\ tale, cioè un editor difficile da usare. Ma ci vuole di
più a dirsi che a farsi. Le scorciatoie da tastiera si imparano in fretta e da
questo se ne trae anche un grosso vantaggio.
...
Ai già citati manuali ufficiali rimandiamo il lettore alla mailing list dei vari pacchetti
