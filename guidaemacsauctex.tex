...
\ProvidesFile{guidaemacsauctex.tex}[2014/01/28 v.1.1.5 Breve guida all'uso di Emacs per LaTeX]
...
perché inizialmente non esiste ed in particolare per:
\begin{blockdescription}[leftmargin=*]
\item[Windows XP] va messo in\\
      \verb|C:\Documents and Settings\|\meta{nome utente}\verb|\Dati Applicazioni\.emacs.d\init.el|;
\item[Windows Vista/7] va messo in:\\
      \verb|C:\Users\|\meta{nome utente}\verb|\AppData\.emacs.d\init.el|;\\
      \verb|C:\Users\|\meta{nome utente}\verb|\AppData\.emacs|;
\item[GNU/Linux, Mac~OS] va messo in \\
      \verb|~/.emacs|\\
      \verb|~/.emacs.el|\\
...
\verb|M-x package-install RET auctex| aggiungendo soltanto la seguente riga al
file \filestyle{.emacs:}
\begin{lstlisting}
(package-initialize)
\end{lstlisting}
...
utenti\footnote{I package installati da ELPA si trovano nella cartella
  \filestyle{~/.emacs.d} e di conseguenza sono disponibili solo per l'utente che
  li installa.} bisognerà valutare di volta in volta se i vantaggi dell'uso di
ELPA superano gli svantaggi. Per installare \auctex{} senza usare ELPA è
necessario utilizzare il programma \progstyle{make}, che viene installato
insieme al correttore ortografico \progstyle{cocoAspell}, descritto nel
paragrafo~\ref{ml:sec:aspell}. Ecco le istruzioni.
