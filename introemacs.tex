...
\ProvidesFile{introemacs.tex}[2013/01/08 v.0.0 Breve guida all'uso di Emacs per LaTeX]
...
% \begin{tabular}{r@{~}c@{~}l}
%   \textsc{Mosè Giordano}   & \textbullet & \textcolor{red}{\texttt{nome dot cognome at dominio dot it}} \\
%   \textsc{Orlando Iovino}  & \textbullet & \texttt{orlando dot iovino at yahoo dot it}                  \\
%   \textsc{Matteo Leccardi} & \textbullet & \textcolor{red}{\texttt{nome dot cognome at dominio dot it}} \\
% \end{tabular}
...
All'indirizzo \url{http://ftp.gnu.org/pub/gnu/emacs/windows/} si
scarica l'archivio compresso \progstyle{emacs-24.2-bin-i386.zip} e lo si
decomprime in una posizione di comodo; una buona soluzione, per
esempio, è decomprimere il suddetto archivio in
\directory{C:/Programmi/Emacs/} che assumeremo, da questo momento,
come cartella di installazione.

Se si vuole creare un collegamento di \emacs{} nel menù Start, è
sufficiente eseguire il programma \progstyle{addpm.exe} contenuto nella
cartella \directory{C:/Programmi/Emacs/bin}.

A questo punto \emacs{} è pronto per scrivere documenti in \LaTeX:
l'utente infatti non deve specificare nessun percorso circa gli
eseguibili della distribuzione \LaTeX\ installata perché \emacs{} li
trova da solo.

\section{Installare AUC\TeX}
\label{oi:sec:installauctex}

Si scarica l'archivio compresso \progstyle{auctex-11.87-e24.2-msw.zip}
dall'indirizzo
\url{http://www.gnu.org/software/auctex/download-for-windows} e lo si
decomprime in \directory{C:/Programmi/Emacs/}.  Si noti che alcuni
file sono gli stessi, perciò basta unire il contenuto dell'archivio di
\auctex{} con quello di \emacs.

\section{Correttore ortografico}
\label{oi:sec:aspell}

Per usare il correttore ortografico in \emacs{} si deve installare il
programma \progstyle{GNU Aspell} disponibile all'indirizzo
\url{http://aspell.net/win32/}. L'ultima versione stabile è la 0.50-3
...
di \progstyle{Aspell} nella variabile d'ambiente
\textsf{PATH}. Secondo, aggiungere al proprio file \filestyle{.init}
le seguenti righe:
\begin{lstlisting}
...
\end{lstlisting}
dove, la prima e la terza sono quelle strettamente necessarie, mentre
la seconda riga serve a risolvere dei problemi con le versioni
precedenti di \emacs.
...
Prima di attivare la ricerca diretta inversa, si spiega
come risolvere il problema dei diritti di amministratore --- qualora
l'account utente ne sia fornito ---, per sistemi operativi Vista e Seven
(su Xp non dovrebbero esserci problemi, per Windows 8 non è stato
possibile provarlo). Infatti, in tale situazione è possibile avere
problemi con la cartella \emph{server} del tipo
\begin{lstlisting}[keywordstyle=\color{black}]
error: The directory '~/.emacs.d/server' is unsafe
\end{lstlisting}

Una possibile soluzione, riportata in~\cite{setupwindows}, è la
seguente. Si deve aprire il prompt dei comandi e portarsi nella
cartella che contiene, a sua volta, la cartella \emph{server}
scrivendo nella riga di comando un'istruzione del tipo
...
takeown /f server
\end{lstlisting}
Se l'operazione sarà andata a buon fine verrà segnalato.

A questo punto, si può configurare \emacs{} per la ricerca diretta
inversa con il visualizzatore \progstyle{SumatraPDF}, che si scarica dal sito
\url{http://blog.kowalczyk.info/software/sumatrapdf/free-pdf-reader.html}.

Per la ricerca inversa si deve aprire \progstyle{SumatraPDF}, andare nel menu
\emph{Impostazioni} e quindi scegliere \emph{Opzioni...} e scrivere la riga
\begin{Verbatim}
C:/Programmi/Emacs/bin/emacsclientw.exe +%l "%f"
\end{Verbatim}
in \emph{Imposta la ricerca inversa via riga di comando}, come si vede
dalla figura~\ref{oi:fig:sumatra:setup}.
\begin{figure}[t]
  \centering
...
% DA CAMBIARE
%
Per la ricerca diretta si deve scaricare il file
\filestyle{sumatra-forward.el} alla pagina
\url{http://william.famille-blum.org/software/sumatra/sumatra-forward.el}
e copiarlo in una delle cartelle accessibili a \emacs{}. Si suggerisce
di copiarlo in una cartella locale, che non dipende
dall'installazione, ad esempio
\begin{lstlisting}
C:/Users/£\meta{nome utente}£/Emacs  
\end{lstlisting}
che va evidentemente creata. Si aggiunge, infine, il codice
\begin{lstlisting}
(add-to-list 'load-path "C:/Users/£\meta{nome utente}£/Emacs")
(require 'sumatra-forward)
\end{lstlisting}
nel file \filestyle{.init}.
...
\begin{longtable}{>{\ttfamily}l>{\ttfamily}lp{0.32\textwidth}}
  ...
  \caption{Scorciatoia è la combinazione di tasti da premere per ottenere il
    ...
    funzione, da eseguire con \texttt{M-x}, a cui è legata a quella combinazione
    di tasti.}
