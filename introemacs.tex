...
lo si decomprime in \directory{C:/Programmi/emacs/}.  Sia nell'archivio di
\auctex{}
sia in quello di \emacs, posizionato in precedenza nel percorso suddetto, vi è
una cartella dal nome \emph{site-lisp}: i file contenuti in queste cartelle
vanno uniti in una sola cartella.
...
Prima di spiegare come attivare la ricerca diretta inversa, si spiega
come risolvere il problema dei diritti di ammonistratore -- qualora
l'account utente ne sia fornito --, per sistemi Windows Vista e Seven
...
Un modo per risolvere il problema, riportato in~\cite{setupwindows}, è
il seguente. Si deve aprire il prompt dei comandi e portarsi nella
cartella che contiene, a sua volta, la cartella \emph{server} scirvendo nella
riga di comando un'istruzione del tipo
...
inversa con il visualizzatore SumatraPDF, scaricabile da sito
\url{http://blog.kowalczyk.info/software/sumatrapdf/free-pdf-reader.html}.

Per la ricerca inversa si deve aprire SumatraPDF, andare nel menu
\texttt{Impostazioni} e quindi scegliere \texttt{Opzioni...} e scrivere la riga
\begin{Verbatim}
C:/Programmi/Emacs/bin/emacsclientw.exe +%l "%f"
\end{Verbatim}
in \emph{Imposta la ricerca inversa via riga di comando}. La figura~\ref{oi:fig:sumatra:setup}
