...
C:/Programmi/emacs/
\end{Verbatim}
La cartella appena creata contiene i file riportati
in~\ref{tab:fileemacs}.
\begin{table}[t]
  \centering
  \begingroup\ttfamily
  ...
  \caption{Contenuto dell'archivio di \emacs{} per Windows}
  \phantomsection\label{tab:fileemacs}
\end{table}
...
Dal sito ufficiale di \auctex{} per Windows (\href{%
  http://www.gnu.org/software/auctex/download-for-windows.html}%
{\mano{} \auctex}), si scarica l'archivio compresso \href{%
  http://ftp.gnu.org/pub/gnu/auctex/auctex-11.86-e23.2-msw.zip}%
{\textsf{auctex-11.86-e23.2-msw.zip}} e lo si decomprime in
\begin{Verbatim}
C:/Programmi/emacs/
\end{Verbatim}
Sia nell'archivio di \auctex{} (tabella~\ref{tab:fileauc}), che quello
di \emacs{} (tabella~\ref{tab:fileemacs}), posizionato in precedenza
nel percorso suddetto, vi è una cartella dal nome \texttt{site-lisp}:
i file contenuti in queste cartelle vanno uniti in una sola cartella.
...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    \bottomrule
  \end{tabular}\endgroup
  \caption{Contenuto dell'archivio di \auctex{} per Windows}
  \phantomsection\label{tab:fileauc}
\end{table}
...
\section{Personalizzare \emacs{} } % titolo cambiato per avere un
...
usare il programma \textsf{GNU Aspell}
(\href{http://aspell.net/win32/}{\mano{} \textsf{GNU Aspell - Win32
    version}}). L'ultima versione stabile per Windows, seppur datata
Dicembre 2002, è la \textsf{0.50-3}.
...
visualizzatore capace di fare questo è SumatraPDF, \textcolor{red}{in
  ambiente Linux si può usare [\ldots]}, mentre in ambiente Mac OS si
usa Skim. 
...
\textsf{SumatraPDF}
(\href{http://blog.kowalczyk.info/software/sumatrapdf/free-pdf-reader.html}{\mano{}~SumatraPDF}).
...
\item si scarica l'eseguibile \textsf{SumatraPDF-TeX}
  (\href{http://william.famille-blum.org/software/sumatra/}{\mano{}~SumatraPDF
    for \TeX\ users}) di William Blum che ha sviluppato appunto la
  funzionalità di ricerca diretta-inversa e fornito un modo per
  accedervi;
...
A queste operazioni segue la configurazione di \textsf{SumatraPDF} e
di \emacs, mediante il proprio file \emph{.emacs}.
...
\textsf{sumatra-forward}
(\href{http://william.famille-blum.org/software/sumatra/sumatra-forward.el}{\mano{}~sumatra-forward.el})
...
\mettifigura{\includegraphics[height=17pt]{by-nc-sa}\hspace{2mm}}\noindent
...
(\mano{}~\href{http://creativecommons.org/licenses/by-nc-sa/3.0/deed.it}{CC
  By-Nc-Sa}).
\end{document}
